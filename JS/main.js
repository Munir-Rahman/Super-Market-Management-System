...
productQuantity.addEventListener('keypress', (btn) => {
  if(btn.key === "Enter"){
    addProductBtn.click();
  }
})
...
function saveProduct(product) {
  ...
  addNotification("جنس اضافه کړل سو", 'success');
...
function deleteProduct(index) {
  if (confirm("ایا تاسو غواړی چي جنس ډیلیټ کړی!")) {
    products.splice(index, 1);
    localStorage.setItem("products", JSON.stringify(products));
    displayProducts();
    addNotification("جنس ‌ډیلیټ کړل سو", 'success');
  }
}
...
function displayProducts() {
  ...
  // Add table headers
  const tableHeaders = ["نوم", "کټګوري", "قیمت", "مقدار", "تغیرول"];
  ...
  filteredProducts.forEach((product, index) => {
    ...
    const cells = [
      ...
      product.category,
      `${product.price} AFG`,
      `<span class="${product.quantity <= 10 ? "status-badge low-stock" : ""
      }">${product.quantity}</span>`,
      `<button onclick="openEditModal(${index})" class="edit-btn">اپډیټ</button>
            <button onclick="deleteProduct(${index})" class="delete-btn">‌ډیلیټ</button>`,
    ];
...
addProductBtn.addEventListener('click', (e) => {
  ...
  if (!name || !category || isNaN(price) || isNaN(quantity)) {
    showWarningAlert('!معلومات په کامیابی سره اپډیټ سول');
    return;
  }
